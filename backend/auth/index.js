...
module.exports = function (httpRequestsTotal, dbConfig) {
    ...
    router.post('/login', async (req, res) => {
        ...
        try {
            ...
            // Crear y almacenar la sesión del usuario
            const sessionId = uuidv4();
            const sessionExpiresAt = new Date(Date.now() + 1000 * 60 * 30); // 30 minutos
            ...
            // Crear y enviar la cookie de sesión
            const sessionData = JSON.stringify({ sessionId, userId: user.id, role: user.role_name });
            const encodedSessionData = Buffer.from(sessionData).toString('base64');
    
            res.cookie('main_session', encodedSessionData, {
                httpOnly: false,
                secure: true,
                sameSite: 'strict',
                maxAge: 1000 * 60 * 30
